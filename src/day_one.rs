...
pub fn puzzle_one() {
    // Puzzle One
    let file = File::open("./input_day_one.txt").unwrap();
    let mut total: u32 = 0;
    let lines = io::BufReader::new(file).lines();
    for line in lines {
        total = total + get_number(line.unwrap());
    }
    println!("The answer for puzzle 1 day 1 is {}", total);
}
...
pub fn puzzle_two() {
    ...
    println!("The answer for puzzle 2 day 1 is {}", total);
}
...
fn get_number_two(line: String) -> u32 {
    ...
    while i <= line.len() && !found {
        ...
        let char = &line[line.len() - i..line.len() - i + 1];
        if char.parse::<u8>().is_ok() {
